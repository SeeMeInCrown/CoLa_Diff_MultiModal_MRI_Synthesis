...
class AttentionBlock(nn.Module):
    ...
    https://github.com/hojonathanho/diffusion/blob/1e0dceb3b3495bbe19116a5e1b3596cd0706c543/diffusion_tf/models/Newnet.py#L66.
    """
...
# overload the '+' operator for coefficients
...
# If they are different, the masks for the left operand are chosen.
    def __add__(self, other):
        result = SCFiltering(self.masks, self.levels, self.sizes[0])
                ...
                for mask2 in range(nummasks):
                    if mask1 == mask2 == 0:
                        temp = 2 * fftconvolve(
                            ...
                            self._upsample(temp, (final_x, final_y), offset))[
                                masklen - 1:-(masklen - 1), (masklen - 1):-(masklen - 1)]
                    else:
                        temp += 2 * fftconvolve(
                            masks[mask1, :, None] * masks[mask2],
                            self._upsample(self[cur_level, mask1, mask2], (final_x, final_y), offset))[
                                masklen - 1:-(masklen - 1), (masklen - 1):-(masklen - 1)]
        return temp
...
def dwt2(img, masks, levels):
    ...
    offset = (masklen + 1) % 2
    filrep = SCFiltering(masks, levels, img.shape)
    filrep[0, 0, 0] = img
    for cur_level in range(1, levels + 1):
        for mask1 in range(nummasks):
            for mask2 in range(nummasks):
                filrep[cur_level, mask1, mask2] = SCFiltering._downsample(
                    2 * fftconvolve(masks[mask1, -1::-1, None] * masks[mask2, -1::-1],
                                    filrep[cur_level - 1, 0, 0]), offset)

    return filrep


class WeightAP(nn.Module):

    def __init__(self, num_channels, epsilon=1e-5, mode='l2', after_relu=False):
        super(WeightAP, self).__init__()
    ...
    def forward(self, x):
        ...
        if self.mode == 'l2':
            ...
            print('Unknown mode!')
        act = 1. + torch.sigmoid(embedding * norm + self.beta)
        return x * act


class NewnetModel(nn.Module):
    """
    The full Newnet model with attention and timestep embedding.
    ...
    :param channel_mult: channel multiplier for each level of the Newnet.
...
class SuperResModel(NewnetModel):
    """
    A NewnetModel that performs super-resolution.
...
class EncoderNewnetModel(nn.Module):
    """
    The half Newnet model with attention and timestep embedding.

    For usage, see Newnet.
    """
...
class SynthesisNetwork(NeuralNetwork):
    def __init__(self):
        super(NeuralNetwork, self).__init__()
    ...
    def _compute_steps_for_sliding_window(patch_size: Tuple[int, ...], image_size: Tuple[int, ...], step_size: float) -> List[List[int]]:
        ...
        for dim in range(len(patch_size)):
            ...
            if num_steps[dim] > 1:
                ...
                actual_step_size = 1000000000000  # does not matter because there is only one step at 0
    ...
    def _internal_predict_3D_3Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
        ...
        if regions_class_order is None:
            predicted_latent = aggregated_results.argmax(0)
        else:
            if all_in_gpu:
                class_probabilities_here = aggregated_results.detach().cpu().numpy()
            else:
                class_probabilities_here = aggregated_results
            predicted_latent = np.zeros(class_probabilities_here.shape[1:], dtype=np.float32)
            for i, c in enumerate(regions_class_order):
                predicted_latent[class_probabilities_here[i] > 0.5] = c
        ...
        return predicted_latent, aggregated_results
    ...
    def _internal_predict_2D_2Dconv(self, x: np.ndarray, min_size: Tuple[int, int], do_mirroring: bool,
        ...
        if regions_class_order is None:
            predicted_latent = predicted_probabilities.argmax(0)
            predicted_latent = predicted_latent.detach().cpu().numpy()
            predicted_probabilities = predicted_probabilities.detach().cpu().numpy()
        else:
            predicted_probabilities = predicted_probabilities.detach().cpu().numpy()
            predicted_latent = np.zeros(predicted_probabilities.shape[1:], dtype=np.float32)
            for i, c in enumerate(regions_class_order):
                predicted_latent[predicted_probabilities[i] > 0.5] = c

        return predicted_latent, predicted_probabilities
    ...
    def _internal_predict_2D_2Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
        ...
        if regions_class_order is None:
            predicted_latent = class_probabilities.argmax(0)
        else:
            if all_in_gpu:
        ...
        if verbose: print("prediction done")
        return predicted_latent, class_probabilities
    ...
    def _internal_predict_3D_2Dconv(self, x: np.ndarray, min_size: Tuple[int, int], do_mirroring: bool,
        ...
        for s in range(x.shape[1]):
            pred_rep, softmax_pres = self._internal_predict_2D_2Dconv(
                x[:, s], min_size, do_mirroring, mirror_axes, regions_class_order, pad_border_mode, pad_kwargs, verbose)
            predicted_latent.append(pred_rep[None])
            softmax_pred.append(softmax_pres[None])
        predicted_latent = np.vstack(predicted_latent)
        softmax_pred = np.vstack(softmax_pred).transpose((1, 0, 2, 3))
        return predicted_latent, softmax_pred
    ...
    def _internal_predict_3D_2Dconv_tiled(self, x: np.ndarray, patch_size: Tuple[int, int], do_mirroring: bool,
        ...
        for s in range(x.shape[1]):
            pred_rep, softmax_pres = self._internal_predict_2D_2Dconv_tiled(
...
class Generic_Newnet(SynthesisNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        """
        super(Generic_Newnet, self).__init__()
        ...
        for d in range(num_pool):
            ...
            if d != num_pool - 1:
                self.SCFiltering.append(SCFiltering(output_features, 256 // (2 **(d+1)), 256 // (2 **(d+2))+1))
        ...
        if self._deep_supervision:
            for ds in range(len(self.conv_blocks_localization)):
                self.rep_outputs.append(conv_op(self.conv_blocks_localization[ds][-1].output_channels, num_classes,
                                                1, 1, 0, 1, 1, rep_output_use_bias))
        else:
            self.rep_outputs.append(conv_op(self.conv_blocks_localization[-1][-1].output_channels, num_classes,
                            1, 1, 0, 1, 1, rep_output_use_bias))
        ...
        self.SCFiltering = nn.ModuleList(self.SCFiltering)
        self.td = nn.ModuleList(self.td)
        self.tu = nn.ModuleList(self.tu)
        self.rep_outputs = nn.ModuleList(self.rep_outputs)
        if self.upscale_logits:
    ...
    def forward(self, x, hs):
        ...
        for u in range(len(self.tu)):
            ...
            if self._deep_supervision:
                rep_outputs.append(self.final_nonlin(self.rep_outputs[u](x)))
        if not rep_outputs:
            rep_outputs.append(self.final_nonlin(self.rep_outputs[0](x)))

        if self._deep_supervision and self.do_ds:
            return tuple([rep_outputs[-1]] + [i(j) for i, j in
                                              zip(list(self.upscale_logits_ops)[::-1], rep_outputs[:-1][::-1])])
        else:
            return emb, rep_outputs[-1]
